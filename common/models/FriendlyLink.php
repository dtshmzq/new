...
class FriendlyLink extends ActiveRecord
{
    ...
    {
        if($this->image !== "0") {//为0表示需要删除图片，Util::getInstance()->handleModelSingleFileUpload()会有判断删除图片
    ...
    {
        Util::getInstance()->handleModelSingleFileUpload($this, 'image', $insert, '@friendlylink/');
    ...
    {
        if( !empty( $this->image ) ){
            Util::getInstance()->deleteThumbnails(Yii::getAlias('@frontend/web/') . str_replace(Yii::$app->params['site']['url'], '', $this->image), [], true);
        }
