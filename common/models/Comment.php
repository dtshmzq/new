...
class Comment extends \yii\db\ActiveRecord
{
    ...
    public function getCommentByAid($id)
    {
        ...
        $list = self::find()->select(['id', 'nickname', 'content', 'created_at'])->where($where)->orderBy("id desc,reply_to desc")->cache(8200)->all();
        $newList = [];
        foreach ($list as $row) {
            $v = $row->getAttributes();
            if ($v['reply_to'] == 0) {
    ...
    {
        if ($insert) {
            if (Yii::$app->feiber->website_comment) {
                if (empty($this->status)) {
                    if (Yii::$app->feiber->website_comment_need_verify) {
