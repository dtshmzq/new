...
class TagIndexImages extends \common\models\TagIndex
{
    ...
    {
//        $result = $this->find()->alias('amt')->select('tag')->leftJoin('tags as at', 'amt.tid = at.id')
//                       ->where(['key' => $this->keyName])->andWhere(['aid' => $aid])->asArray()->all();
        $result = [];
        $result = ArrayHelper::getColumn($result, 'tag');
