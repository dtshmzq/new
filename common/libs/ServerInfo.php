...
class ServerInfo
{
    ...
    public function getInfo()
    {
        ...
        switch (PHP_OS) {// 根据不同系统取得CPU相关信息
            case "Linux":
                $sysInfo = self::sysLinux();
                break;
            case "FreeBSD":
                $sysInfo = self::sysFreebsd();
                break;
            default:
                $sysInfo = [];
                break;
    ...
    {
        //CPU
        if (false === ($res['cpu']['num'] = self::getKey("hw.ncpu"))) {
            return false;
        }
        $res['cpu']['model'] = self::getKey("hw.model");
        //LOAD AVG
        if (false === ($res['loadAvg'] = self::getKey("vm.loadavg"))) {
            return false;
        }
        //UPTIME
        if (false === ($buf = self::getKey("kern.boottime"))) {
            return false;
        }
        $buf = explode(' ', $buf);
        ...
        $res['uptime'] .= $min . "分钟";
        //MEMORY
        if (false === ($buf = self::getKey("hw.physmem"))) {
            return false;
        }
        $res['memTotal'] = round($buf / 1024 / 1024, 2);

        $str = self::getKey("vm.vmtotal");
    ...
    {
        return self::doCommand('sysctl', "-n $keyName");
    }
    ...
    //执行系统命令 FreeBSD
    private function doCommand($commandName, $args)
    {
        $buffer = "";
        if (false === ($command = self::findCommand($commandName))) {
            return false;
        }
