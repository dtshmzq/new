...
class BannerService extends Service implements BannerServiceInterface
{
    ...
    public function updateBanner($id, $sign, array $postData = [])
    {
        ...
        if( $banner->load($postData) && $banner->validate() ){
            ...
            foreach ($banners as $b){
                /** @var BannerForm $b */
                if( $b->sign === $sign ){
                    Util::getInstance()->handleModelSingleFileUpload($b, 'img', false, '@uploads/setting/banner/');
                    $banner->img = $b->img;
    ...
    public function createBanner($id, array $postData = [])
    {
        ...
        if ($banner->load($postData) && $banner->validate()) {
            $model = Options::findOne($id);
            $banners = $this->getBanners($id);
            Util::getInstance()->handleModelSingleFileUpload($banner, 'img', true, '@uploads/setting/banner/');
    ...
    public function deleteBanner($id, $sign)
    {
        ...
        foreach ($banners as $banner) {
            ...
            if ($banner->sign === $sign) {
                if( !empty( $banner->img ) ){
                    Util::getInstance()->deleteThumbnails(Yii::getAlias('@frontend/web') . $banner->img, [], true);
                }
                continue;
            }
            $newBanners[] = $banner->getValue();
