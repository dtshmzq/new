...
use Yii;
use common\components\FeiberInit;
use yii\helpers\Url;
...
class SNSView extends \yii\base\Widget
{
    ...
    public function run()
    {
        /** @var FeiberInit $feiber */
        $feiber = Yii::$app->get("feiber");
        $wechatImg = Yii::$app->getRequest()->getBaseUrl() . "/static/images/weixin.jpg";
        $template = str_replace("{%WEIBO%}", $feiber->weibo, $this->template);
        $template = str_replace("{%FACEBOOK%}", $feiber->facebook, $template);
        $template = str_replace("{%WECHAT%}", $feiber->wechat, $template);
        $template = str_replace("{%WECHAT_IMG%}", $wechatImg, $template);
        $template = str_replace("{%EMAIL%}", $feiber->email, $template);
        $template = str_replace("{%FOLLOW_WECHAT%}", Yii::t("frontend", "Follow Wechat"), $template);
        $template = str_replace("{%QQ%}", $feiber->qq, $template);
