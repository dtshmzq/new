...
class AdForm extends \common\models\Options
{
    ...
    public function beforeValidate()
    {
        if( $this->type === Constants::AD_IMG ) {;
            if ($this->ad !== "0") {//为0表示需要删除图片，Util::getInstance()->handleModelSingleFileUpload()会有判断删除图片
    ...
    public function beforeSave($insert)
    {
        ...
        if( $this->input_type == Constants::AD_TEXT ){
            ...
            if( $oldInput != Constants::AD_TEXT ){//delete origin advertisement file
                $text = $this->ad;
                Util::getInstance()->handleModelSingleFileUploadAbnormal($this, 'ad', $this->advertisementFilePath, $this->getOldAttribute('ad'), ['deleteOldFile'=>true]);
                $this->ad = $text;
            }
        }else {
            Util::getInstance()->handleModelSingleFileUploadAbnormal($this, 'ad', $this->advertisementFilePath, $this->getOldAttribute('ad'));
        }
