...
class TimeSearchBehavior extends \yii\base\Behavior
{
    ...
    {
        parent::init();
        empty($this->timeAttributes) &&
        $this->timeAttributes = [$this->createdAtAttribute => $this->createdAtAttribute, $this->updatedAtAttribute => $this->updatedAtAttribute];
    }
    ...
    public function beforeSearch($event)
    {
        ...
        foreach ($this->timeAttributes as $filed => $attribute) {
            ...
            if (!empty($timeAt)) {
                ...
                $event->query->andFilterWhere(['between', $filed, $startAt, $endAt]);
            }
        }
    }
}