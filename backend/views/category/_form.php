    ...
    <div class="col-sm-12">
        <div class="ibox">
            ...
            <div class="ibox-content">
                ...
                ?>
                <?= $form->field($model, 'parent_id')->label(Yii::t('app', 'Parent Id'))->dropDownList($categories, ['options' => $disabledOptions]) ?>
                <div class="hr-line-dashed"></div>
                <?= $form->field($model, 'name')->textInput(['maxlength' => 64]) ?>
                <div class="hr-line-dashed"></div>
                <?= $form->field($model, 'alias')->textInput(['maxlength' => 64]) ?>
                <div class="hr-line-dashed"></div>
                <?= $form->field($model, 'seo_title')->textInput(['maxlength' => 150]) ?>
                <div class="hr-line-dashed"></div>
                <?= $form->field($model, 'seo_keywords')->textInput(['maxlength' => 50]) ?>
                <div class="hr-line-dashed"></div>
                <?= $form->field($model, 'seo_description')->textInput(['maxlength' => 255]) ?>
                <div class="hr-line-dashed"></div>
                <?= $form->field($model, 'sort')->textInput(['maxlength' => 512]) ?>
                <div class="hr-line-dashed"></div>
                <?= $form->field($model, 'template')->chosenSelect(Util::getInstance()->getViewTemplate("category")) ?>
                <div class="hr-line-dashed"></div>
                <?= $form->field($model, 'article_template')->chosenSelect(Util::getInstance()->getViewTemplate()) ?>
                <div class="hr-line-dashed"></div>
