                        ...
                        'columns'      => [
                            ...
                            [
                                ...
                                'value'     => function($model, $key, $index, $column) use ($frontendURLManager) {
                                    ...
                                    $url = $frontendURLManager->enablePrettyUrl ? Yii::$app->params['site']['url'] . "/{$scriptName}view/{$model->id}.html"
                                        : Yii::$app->params['site']['url'] . '/index.php?r=article/view&id=' . $model->id . $frontendURLManager->suffix;
                            ...
                            [
                                ...
                                'value'     => function($model, $key, $index, $column) {
                                    ...
                                    return Html::a(
                                        Constants::getArticleStatus($model['status']),
                                        ['state', 'id' => $model['id']],
                                        [
                                            'class'        => 'btn btn-xs btn-rounded ' .
                                                              ($model['status'] == Constants::YesNo_Yes ? 'btn-info' : 'btn-default'),
                                            'data-confirm' => $model['status'] == Constants::YesNo_Yes ? Yii::t(
                                                'app',
                                                'Are you sure you want to cancel release?'
                                            ) : Yii::t('app', 'Are you sure you want to publish?'),
                                            'data-method'  => 'post',
                                            'data-pjax'    => '0',
                                            'data-params'  => [
                                                $model->formName() . '[status]' => $model['status'] == Constants::YesNo_Yes ? Constants::YesNo_No
                                                    : Constants::YesNo_Yes,
                                            ],
                                        ]
                                    );
